...
contract QuestContract {
    ...
    IERC20 public token;
    constructor(address _userContract, address _tilecontractaddress, address _computecontract, address _token) {
        admin = msg.sender;
    ...
    function fetchRegionInfo(uint256 regionId) internal view returns (
        uint8 pollution,
        uint8 fertility,
        uint8 waterlevel,
        uint8 eco,
    ...
    function buildFactory(uint256 regionId, uint8 tileId, uint8 _factoryTypeId) external {
        (, bool isBeingUsed, , , , , , ) = fetchTileInfo(regionId, tileId);
...
// updateWFG(uint32 tileId, bool worf, uint8 growth, uint256 regionId, address _user)
    ...
    function produceFromFactory(uint256 regionId, uint32 tileId, uint8 _factoryTypeId) external {
        (, bool isBeingUsed, bool isCrop , , uint8 factoryTypeId, , , ) = fetchTileInfo(regionId, tileId);
